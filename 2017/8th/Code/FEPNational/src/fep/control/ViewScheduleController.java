...
public class ViewScheduleController implements Initializable {
	 ...
	 * @version 3.0
	 * 
	 * This is a controller class. This class handles all the actions of the View schedule window. 
	 */
	 ...
	 * @throws IOException
	 * 
	 * This method populates the table with the employee schedule of the selected week 
	 * of the given year. 
	 */
	public void btnView(ActionEvent event) throws IOException {
		...
		try {
			 ...
			 * @see package table
			 */
			String query = "select * from '" + comboYear.trim() + textWeek.substring(0, 4) + textWeek.substring(5)
					+ "'";
			PreparedStatement pst = loginModel.connection.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
			...
			status.setText("Schedule displayed!");
